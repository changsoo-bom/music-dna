...
/**
 * **제목은 사이트 이름과 화면 이름을 둘 다 말한다.**
 *
 * 루트에 순수 문자열을 두면 페이지의 `title` 이 그것을 통째로 갈아치워서
 * 탭에 "둘러보기" 만 남는다 — 창을 여러 개 띄운 사람은 어느 탭이 이 사이트인지
 * 알 수 없다. 그렇다고 페이지 제목을 다 지우면 반대쪽이 무너진다: 네 화면이
 * 전부 같은 문자열로 색인되고, 이번에는 **어느 탭이 보관함인지**를 모른다.
 *
 * `template` 이 그 둘을 같이 준다. 페이지는 화면 이름만 적고(`title: "둘러보기"`),
 * 사이트 이름은 여기서 한 번만 붙는다. `default` 는 자기 제목이 없는 화면
 * (홈)이 쓴다 — `template` 은 루트 자신에게는 안 걸린다.
 */
export const metadata: Metadata = {
  title: {
    default: "MUSIC DNA — 당신의 취향에는 지문이 있습니다",
    template: "%s · MUSIC DNA",
  },
  description:
